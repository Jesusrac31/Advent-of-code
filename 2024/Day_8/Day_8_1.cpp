  ...
  for (auto it = ubicaciones.begin(); it != ubicaciones.end(); it++){
    for (int i = 0; i<(*it).second.size(); i++){
        for (int j = i; j<(*it).second.size(); j++){
            if (i != j){
                pair_el.first = 2*(*it).second[i].first-(*it).second[j].first;
                pair_el.second = 2*(*it).second[i].second-(*it).second[j].second;
                if (pair_el.first>=0 && pair_el.first<mapa.size() && pair_el.second>=0 && pair_el.second<mapa[0].size()){
                    nodes.insert(pair_el);
                    mapa[pair_el.first][pair_el.second] = '#';
                }
                pair_el.first = 2*(*it).second[j].first-(*it).second[i].first;
                pair_el.second = 2*(*it).second[j].second-(*it).second[i].second;
                if (pair_el.first>=0 && pair_el.first<mapa.size() && pair_el.second>=0 && pair_el.second<mapa[0].size()){
                    nodes.insert(pair_el);
                    mapa[pair_el.first][pair_el.second] = '#';
                }
